...
contract TestHook {
    ...
    IV4Router public constant router = IV4Router(0xA51afAFe0263b40EdaEf0Df8781eA9aa03E381a3);
    ...
    function openOrder() public view returns (bytes memory, bytes[] memory, bytes memory) {
        ...
        uint256 liquidity = true
            ? LiquidityAmounts.getLiquidityForAmount0(
                tickLower.getSqrtPriceAtTick(), tickUpper.getSqrtPriceAtTick(), 100000000000
            )
    ...
    function swap() public view returns (bytes memory commands, bytes[] memory callData) {
        bytes[] memory _params = new bytes[](4);

        _params[0] = abi.encode(IV4Router.ExactOutputSingleParams(key, false, 250000000000, 1e6, ""));

        _params[1] = abi.encode(Currency.wrap(0xaf88d065e77c8cC2239327C5EDb3A432268e5831), 1e4, true);
        _params[2] = abi.encode(Currency.wrap(address(0)), 0);
        _params[3] = abi.encode(Currency.wrap(0xaf88d065e77c8cC2239327C5EDb3A432268e5831), 0);
